...
- Q: When should the planner submit the first trip for verification? → A: After a minimum set: Skolwin's location, one legal path, at least one travel method, and resource costs for that path. **Superseded by Q4 and FR-023**, which also require allowed commands before the first check.
...
### Session 2026-08-15 (readiness gate)

- Q: Promote plan-only wire/ops rules into the spec? → A: Yes — discover POST/path/body, first `/api/toolsearch`, verify `task=savethem`, secret names, and exit codes 0/1/2 belong in the spec
- Q: Hub timeout or unreachable? → A: Same as empty/unreadable: one failed use, then retry or stop at the limit
- Q: How is FR-023's first-verify gate judged? → A: Planner judgment from discovery text is enough; no extra parser
- Q: Are 30/10 uses fixed caps? → A: No — configuration is authoritative; 30 and 10 are defaults only
- Q: Vehicle-to-vehicle switch without walking? → A: No — disband to foot only
- Q: `{FLG:...}` on a discover response? → A: Impossible; no requirement
...
3. **Given** the planner has already used the configured discovery limit (default 30), **When** it would send another, **Then** the program shuts down immediately and reports that the discovery limit was reached.
...
**Independent Test**: Feed a rejected trip and its error into the planner. Confirm the next submitted plan is different and addresses the reported problem, and that exhausting the configured verify limit causes immediate shutdown.
...
3. **Given** the configured verification limit (default 10) has been used, **When** the flag has not been obtained, **Then** the program shuts down immediately and reports that the verification limit was reached.
4. **Given** a trip that still has a vehicle the planner no longer needs, **When** the planner revises the plan, **Then** it may disband that vehicle and continue the remaining legs on foot. It MUST NOT switch to a different vehicle.
...
- The program reaches the configured discovery or verification limit: it exits at once even if the other service still has remaining uses.
- The flag arrives on a verification response: the program prints it and exits without spending leftover discovery or verification uses. A `{FLG:...}` string is not expected on a discovery response; no handling is required.
- Required secrets are missing at start: the program stops immediately with exit status 2, reports the failure, and does not call game services or the planner model.
- A service response is empty, unreadable, timed out, or unreachable: the program treats it as a failed use of that service, reports the problem, and either retries with a changed request (if uses remain) or exits if the limit is reached.
- A verify `answer` is missing, empty, or not a list of strings: the program rejects it locally, does not call the hub, and does not decrement the verify budget.
...
- **FR-007**: The program MUST NOT send more discovery requests than the configured discover limit in one run (default 30). Configuration is authoritative. Reaching that limit without the flag MUST shut the program down immediately.
- **FR-008**: The planner MUST produce a trip plan whose goal is to move the player to the city of Skolwin.
- **FR-023**: The planner MUST NOT submit the first verification until it has Skolwin's location, one legal path to that city, at least one travel method, resource costs for that path, and the allowed commands for writing a trip. It MUST NOT delay that first check solely to finish a full 10 by 10 map or a complete vehicle catalog. The planner MAY treat those five facts as met when it judges them present in discovery text; no additional parser is required.
...
- **FR-013**: The planner MUST be able to disband a vehicle at any plan step and continue later steps on foot. The planner MUST NOT switch from one vehicle to another vehicle. A trip MAY start in at most one vehicle, then optionally continue on foot.
...
- **FR-025**: Discovery requests MUST be HTTP POST to `{hub base URL}{path}` with JSON body `apikey` (injected `HUB_API_KEY`, never from the planner) and `query` (English, from the planner). The first discovery path in a run MUST be the configured toolsearch path (default `/api/toolsearch`). Later paths MUST come from discovery. Default hub base URL is `https://hub.ag3nts.org`.
- **FR-026**: Verification requests MUST be HTTP POST to `{hub base URL}{verify path}` (default `/verify`) with JSON body `apikey` (injected `HUB_API_KEY`), `task` (from configuration, default `savethem`), and `answer` (string array of discovered commands). The planner MUST NOT supply `apikey` or `task`. The first `answer` element MAY be a vehicle type.
- **FR-027**: The process MUST exit `0` when the flag is acquired, `1` when a tool limit is reached or planning ends without a flag, and `2` when required secrets are missing or blank.
- **FR-028**: There is no wall-clock or extra planner-turn cap. The run MUST stop only when the flag is acquired or a configured tool limit is reached (or startup fails per FR-021).
- **FR-029**: A missing, empty, or non-list verify `answer` MUST be rejected locally: no hub call and no verify-budget decrement.
...
- **Vehicle**: A discovered travel method with its own fuel use. A trip MAY start in at most one vehicle. It may be abandoned so the player continues on foot. Vehicle-to-vehicle change is not allowed.
...
- **Flag**: The game prize string returned only by a successful verification. Not expected on discovery responses.
- **Discovery service (`discoverTool`)**: Hub POST described in FR-025.
- **Verification service (`verifyTool`)**: Hub POST described in FR-026.
...
- **SC-002**: A winning run obtains the flag without exceeding the configured discovery and verification limits (defaults 30 and 10).
...
- **SC-006**: In a winning trip, the player arrives at Skolwin with fuel and food still above zero. A `{FLG:...}` from verification is sufficient evidence; the program MUST NOT require a separate resource meter.
...
- A trip may start in one discovered vehicle and later disband to foot. Vehicle-to-vehicle change is not allowed.
- Configured discover and verify limits default to 30 and 10; those values may be changed in configuration and then bind the run.
- There is no extra time or model-turn cap beyond the tool budgets and startup failure.
- FR-023's five facts are met when the planner judges them present in discovery text.
